...
const isAuthorized = (requiredRoles, restrictedTo) => {
    return (request, response, next) => __awaiter(void 0, void 0, void 0, function* () {
        ...
        if (!hasRequiredRole(request.user.role, requiredRoles)) {
            return response.status(response_enums_1.ResponseStatus.Forbidden).json({
                message: response_enums_1.ResponseMessages.Forbidden,
            });
        }
        try {
            if (yield hasAnyRestriction(request.user.userId, restrictedTo)) {
                return response
                    .status(response_enums_1.ResponseStatus.Forbidden)
                    .send({ message: response_enums_1.ResponseMessages.Forbidden });
            }
        }
        catch (error) {
...
const hasAnyRestriction = (userId, restrictedTo) => __awaiter(void 0, void 0, void 0, function* () {
    if (!restrictedTo)
        return false;
    const userRestrictions = yield restrictionModel.getUserRestrictions(userId);
    return userRestrictions.some((restriction) => restrictedTo.includes(restriction.restrictionType));
});
